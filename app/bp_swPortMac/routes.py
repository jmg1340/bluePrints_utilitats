...
def buscaMAC( mac, ipSW, nomSW, index_fila, event ):
	...
	for intent in range(1,4):    # 3 intents
			socketioApp.emit('recepcioDades', {'fila': index_fila, 'text': "Recuperant informació del switch ...", 'colorBg': "yellow", "colorText": "black"})
			...
			try:    
					...
					if respostaCapturaTextURL.status_code == 200:
							...
							if coincidencia:
									socketioApp.emit('recepcioDades', {'fila': index_fila, 'text': coincidencia, 'colorBg': "green", "colorText": "black"})
							else:
	...
	socketioApp.emit('tasquesPendents', f"Pendents: {len(threading.enumerate()) - 2}" )
	



def coincidencies( self, txtURL, txtMAC ):
	arrLinies = txtURL.split("\n")
	for linia in arrLinies:
		if txtMAC.lower() in linia.lower():
			...
			if linia.startswith("up") or linia.startswith("down"):
				print( "LINIA: ", linia )
				return linia
